...
export function DesktopWindow({
  ...
  // Genie target = the dock, at the bottom-center of the viewport. On minimize/
  // close the window scales down and slides into it (origin bottom-center), the
  // macOS "suck into dock" read.
  ...
  return (
    <motion.section
      ...
      className={cn(
        "dz-solidify absolute flex flex-col overflow-hidden rounded-[14px] border backdrop-blur-2xl",
    ...
    >
      ...
      <header
        ...
        className={cn(
          "relative flex h-9 shrink-0 cursor-grab items-center gap-2 border-b border-line/60 px-3 active:cursor-grabbing",
          frontmost ? "bg-[rgb(var(--ink-raised)/0.5)]" : "bg-transparent"
        )}
      >
        {/* traffic lights */}
        <div className="group/lights z-10 flex items-center gap-[7px]" onPointerDown={(e) => e.stopPropagation()}>
          ...
          <TrafficLight color="#28C840" title="Zoom" onClick={onMaximize} glyph="+" />
        </div>

        {/* centered title (macOS) */}
        <div className="pointer-events-none absolute inset-x-0 flex items-center justify-center gap-1.5">
          <Icon size={12} style={{ color: frontmost ? accent : undefined }} className={cn(!frontmost && "text-fg-faint")} />
          <span className={cn("max-w-[60%] truncate text-xs font-semibold tracking-tight", frontmost ? "text-fg-secondary" : "text-fg-muted")}>
            {win.title}
          </span>
        </div>
      </header>
      ...
      {/* content */}
      <div className={cn("min-h-0 flex-1 overflow-hidden", bare ? "" : "overflow-y-auto overflow-x-hidden p-4")}>{children}</div>
...
function TrafficLight({ color, title, onClick, glyph }: { color: string; title: string; onClick: () => void; glyph: string }) {
  return (
    <button
      title={title}
      aria-label={title}

...
export interface AppEntry {
  ...
  perm?: Permission;
  /** Power-up: route still works, but it's kept out of every launcher surface
   *  (dock, the launcher grid, Spotlight, menubar) to keep the demo to the spine. */
  hidden?: boolean;
}
...
export const APPS: AppEntry[] = [
  ...
  { id: "vault", label: "Knowledge Vault", iconKey: "vault", accent: "#FEBC2E", kind: "panel", dock: false, hidden: true },
  ...
  { id: "r-briefing", label: "Exec Briefing", iconKey: "briefing", accent: "#3DF59E", kind: "iframe", href: "/briefing", dock: false, hidden: true, perm: "view_executive", ...IFRAME_RECT },
  ...
  { id: "r-projects", label: "Projects", iconKey: "projects", accent: "#7C6CFF", kind: "iframe", href: "/projects", dock: true, ...IFRAME_RECT },
  // Org Memory = the merged Intelligence area (Decisions · Capabilities · Learnings).
  // Power-up: lives in Launchpad, not pinned to the dock.
  { id: "r-memory", label: "Org Memory", iconKey: "memory", accent: "#C084FC", kind: "iframe", href: "/memory", dock: false, hidden: true, perm: "view_capacity", ...IFRAME_RECT },
  ...
  { id: "simulation", label: "What-If Simulation", iconKey: "simulation", accent: "#FEBC2E", kind: "panel", dock: false, hidden: true, perm: "view_executive", x: 120, y: 60, w: 920, h: 640 },
...
export const DEFAULT_DOCK = APPS.filter((a) => a.dock && !a.hidden).map((a) => a.id);
/** Apps that may appear in any launcher surface (grid, Spotlight, menubar). */
export const LAUNCHER_APPS = APPS.filter((a) => !a.hidden);

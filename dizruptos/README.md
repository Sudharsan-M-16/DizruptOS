...
|---|---|
| ![Desktop Workspace](./temp%20ss/os-3-desktop.png) | ![Control Center](./temp%20ss/os-4-controlcenter.png) |
...
DizruptOS features a custom window management engine built on React and Zustand (`src/lib/os.ts` & `src/components/desktop/use-desktop.ts`).
...
# 2. Start local development server (Demo Mode — zero config needed)
npm run dev
```
